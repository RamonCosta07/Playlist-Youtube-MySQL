...
INSERT INTO videos (nome_video, autor_video) values ('Páginas com PYTHON', 'Python'); 

SELECT * FROM videos_canais AS vc RIGHT OUTER JOIN videos AS v ON vc.fk_video = v.id_video; -- Vai exibir videos mesmo que não tenha canal

SELECT v.id_video, v.nome_video FROM 
videos_canais AS vc RIGHT OUTER JOIN videos AS v ON vc.fk_video = v.id_video
UNION 
SELECT c.id_canal, c.nome_canal FROM 
videos_canais AS vc RIGHT OUTER JOIN canais AS c ON vc.fk_canal = c.id_canal;

SELECT * FROM videos_canais AS vs JOIN videos AS v
ON vs.fk_video = v.id_video
JOIN canais AS c ON vs.fk_canal = c.id_canal
where c.id_canal = 2;